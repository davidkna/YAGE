$(function ($) {
	...
	$('a[data-toggle="tab"]')
		...
		$.get('http://glitchroutes.com/build/', function (rp) {
			var $rp = $('<div></div>')
				.html($(rp.replace(/<script[ >][\s\S]*?<\/script>/g, '')));
			$rp.find('form')
				.attr('action', 'http://glitchroutes.com/build/')
				.removeAttr('style')
				.wrap('<div class="form"></div>');
			$('#build')
				.append($rp.find(".form")
				.eq(0)
				.html() + '<hr/>' + $rp.find(".form")
				.eq(1)
				.html());
		...
		$.get('http://seriousroutes.com/', function (rp) {
			var div = $('<div></div>'),
				...
				$srs = $('#serious'),
				res = '<div class="accordion" id="accordion"><h3>Top sign routes</h3>',
				index = 0;
			$(div)
				.append($('<div></div>')
				.html($rp)
			...
			$srs.find('form')
				.each(function () {
				$(this)
					.attr('action', 'http://www.glitch.com/locations/' + $(this)
					.attr('action'));
				$(this)
					.find('button')
					.addClass('btn');
